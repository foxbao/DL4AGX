...
const std::vector<std::string>& track_output_names() {
  static const std::vector<std::string> names = {
...
void usage(const char* program) {
  std::fprintf(
      stderr,
      "Usage: %s <sparse.onnx> <frontend.engine> <track.engine> <plugin.so> "
...
uniad_lidar::BevGridSpec bev_grid_from_track_engine(
  ...
  uniad_lidar::require(
      grid.batch_size == 1,
      "uniad_lidar_track currently supports batch size 1.");
  return grid;
}
...
std::vector<float> adapt_middle_bev_for_frontend(
  ...
  if (expected_numel > 0 && middle_bev.size() % expected_numel == 0) {
    ...
    std::printf("[INFO] sparse output has %zu frontend-sized batches; "
                "using batch 0 for single-frame track runtime.\n",
                batches);
...
CliArgs parse_cli(int argc, char** argv) {
  ...
  uniad_lidar::require(!args.metadata_input.empty(),
                       "--metadata-json is required for track runtime.");
  uniad_lidar::require(!args.track_init_dir.empty(),
                       "--track-init-dir is required for track runtime.");
...
int main(int argc, char** argv) {
  ...
  std::shared_ptr<TensorRT::Engine> track_engine =
  ...
  const uniad_lidar::BevGridSpec prev_bev_grid =
      bev_grid_from_track_engine(track_engine);

  const uniad_lidar::TensorMap initial_track_state =
      load_initial_track_state(track_engine, args.track_init_dir,
  ...
  for (int frame = 0; frame < args.num_frames; ++frame) {
    ...
    uniad_lidar::require(metadata.has_timestamp,
                         "Track runtime requires timestamp in metadata.");
    ...
    std::printf("[INFO] frame %d metadata: %s, prev_bev_exists=%s, "
                ...
                "shift=(%.9g, %.9g), prev_bev_aligned=%d\n",
    ...
    for (const std::string& name : track_output_names()) {
    ...
    for (const std::string& name : track_state_input_names()) {
      track_state[name] = coerce_for_track_input(
          track_engine, name, outputs.at(output_name_for_state_input(name)));
    }
