...
struct TrackLidarInput {
  std::vector<float> lidar_bev;
  std::vector<float> prev_bev;
  std::vector<float> shift;
  std::vector<float> use_prev_bev;
  std::vector<float> prev_timestamp;
  std::vector<float> prev_l2g_r_mat;
  std::vector<float> prev_l2g_t;
...
TensorMap run_track_lidar_trt(
    ...
    const std::vector<std::string>& output_names,
    cudaStream_t stream,
    const std::string& label = "LiDAR Track TensorRT",
    bool print_info = true,
